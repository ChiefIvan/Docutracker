...
from .static.predef_function.user_validation import UserValidation, PasswordValidator, Sanitizer, EmailValidator
...
def admin_login():
    ...
    success_message: str = "A Verification Link has been sent to your Email."
    invalid_credential_error = "Invalid Email!"

    data = request.json

    validate = EmailValidator({"email": data["email"]}).validate()
...
def signup() -> dict:
    if request.method == "POST":
        ...
        duplicate_email_error: str = "Email Already Exist!, Please try another one."
        insertion_error: str = "Database Insertion Error Occured!"
...
def captcha_verification():
    if request.method == "GET":
        ...
        captcha: ImageCaptcha = ImageCaptcha(
            width=400, height=220, font_sizes=(80, 90, 100))
